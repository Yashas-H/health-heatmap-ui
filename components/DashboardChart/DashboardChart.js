import { DemographicSeroData } from "./SeroprevalenceData";
...
const BarChart = () => {
  ...
  let [category, indicatorList] = CategoryValueData(DemographicSeroData)
  const [categoryName, setCategoryName] = useState(category[0]);
  const [indicator, setIndicator] = useState(indicatorList[0]);
  const [filter, setFilter] = useState(category[2]);
  const [filterList, setFilterList] = useState(filterTheList(DemographicSeroData, filter));
  const [selectedFilterList, setSelectedFilterList] = useState([filterList[0]]);
  ...
  const [finalPlainGraphData, setFinalPlainGraphData] = useState(
    filterPlainGraph(DemographicSeroData, categoryName, indicator, filter, selectedFilterList, selectedAxisLabel)
  );
  ...
  useEffect(() => {
    setFilterList(filterTheList(DemographicSeroData, filter))
  }, [filter]);
  useEffect(() => {
    setFinalPlainGraphData(
      filterPlainGraph(DemographicSeroData, categoryName, indicator, filter, selectedFilterList, selectedAxisLabel)
    )
  }, [DemographicSeroData, categoryName, indicator, filter, selectedFilterList, selectedAxisLabel]);
  ...
  return (
    <Grid xs={12} container>
      <Grid xs={12} style={{textAlign:'center', marginBottom:'20px'}}>
                ...
                <Typography >
                Compare results from the 1st, 2nd and 3rd round of National serosurveys. 
                </Typography>
            </Grid>
