...
export default function IDSPSidebar({ filter, dispatchFilter }) {
	...
	const { loading: statesLoading, error: statesError, data: states } = useDimensionValues({
		fields: ['entity.State'],
		filter: idspFilter,
	});

	const { loading: diagnosesLoading, error: diagnosesError, data: diagnoses } = useDimensionValues({
					...
					<Select
						...
						onChange={(diagnoses) => {
							if (diagnoses === null)
								...
								dispatchFilter({
									type: 'set-term',
									payload: ['diagnosis.id', diagnoses.map((o) => o['diagnosis.id'])],
								});
						}}
